...
import { X, Pencil, Trash2, RotateCcw, Plus, Link2, AlertTriangle, StickyNote } from 'lucide-react';
...
export default function CompanyDrawer({ companyId, caps, options, initialTab, onClose, onChanged, onOpenCompany }) {
  ...
  useEsc(editing || contactForm ? noop : onClose);
  ...
  async function addNote() {
    ...
    setBusy(true);
    try {
      const about = noteAbout === 'company' ? { entityType: 'company', entityId: companyId } : { entityType: 'contact', entityId: noteAbout };
  ...
  const tabs = [['overview', 'Visão geral'], ['contacts', `Contatos${data ? ` (${data.contacts.length})` : ''}`], ['history', 'Histórico'], ['projects', `Projetos${data ? ` (${data.projects.length})` : ''}`]];
                ...
                <div className="crm-section">
                  ...
                  {data.notes.map((n) => (
                    <div key={n.id} className="crm-note">
                      <div className="crm-note-meta"><span>{n.createdByName || 'Alguém'}{n.contactName ? ` · sobre ${n.contactName}` : ''} · {fmtDateTimeBR(n.createdAt)}</span>

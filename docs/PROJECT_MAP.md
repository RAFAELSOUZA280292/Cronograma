...
server/assistantRetrieval.js  Assistente Inteligente de Projetos — pipeline de 2 chamadas à IA (resolveQuery/synthesizeAnswer/askProjectAssistant), citações validadas contra os chunks recuperados, aprendizados persistidos em ai_project_insights; ProposedActionSchema com 6 tipos de ação (Fase 4) e contexto de Agenda (getConnectionStatus/listEvents); SynthesizeAnswerSchema resposta ESTRUTURADA (introduction/sections/insights no lugar de answer, Fase 5) + flattenStructuredAnswer(); resolveQuery em claude-sonnet-5 (não Opus) + cache_control nos dois system prompts + loadInsights limit 20 (Fase 6, 2026-09-10, redução de custo) — ver PROJECT_CONTEXT.md §27/§32/§34/§35.
...
server/assistant.js       Rotas /api/assistant/* (conversation, ask, conversation/clear, messages/:id/feedback, messages/:id/action, reindex, reindex-needed — Fase 6, 2026-09-10, checagem barata antes do auto-reindex), 2026-09 — ver PROJECT_CONTEXT.md §27/§35.

...
const ProposedActionSchema = z.object({
  type: z.enum([
    ...
    'save_knowledge_fact', 'flag_knowledge_conflict',
  ]).describe('Qual ação está sendo proposta.'),
  subject: z.string().nullable().describe('SÓ pra type="save_knowledge_fact" ou "flag_knowledge_conflict": um rótulo curto do ASSUNTO (ex.: "cargo do Felipe", "rateio do seguro de vida") — usado depois pra detectar se um fato novo conflita/atualiza/complementa um já existente sobre o mesmo assunto. null pros outros tipos.'),
  content: z.string().nullable().describe('Pra type="save_knowledge_fact": o fato em si, como uma frase clara e autossuficiente (ex.: "Felipe é o CEO da PRICETAX"). Pra type="flag_knowledge_conflict": a PRIMEIRA versão do fato em disputa (ex.: "O resumo da reunião registra o rateio do seguro de vida como empresa 90% / funcionário 10%."). null pros outros tipos.'),
  conflictingContent: z.string().nullable().describe('SÓ pra type="flag_knowledge_conflict": a SEGUNDA versão, que contradiz `content` — sempre inclua de onde veio cada uma dentro do próprio texto (ex.: "Na fala às 10:20, Evanio disse que a maior participação é do funcionário, não da empresa."), já que os dois viram fatos rastreáveis separados. null pros outros tipos.'),
  scope: z.enum(['conversation', 'project', 'org']).nullable().describe('SÓ pra type="save_knowledge_fact" ou "flag_knowledge_conflict": sua MELHOR SUGESTÃO de escopo — o usuário ainda vai poder trocar antes de confirmar, então sugira com confiança, não precisa ficar em cima do muro. "conversation" se o fato só faz sentido pra guiar o restante DESTA conversa (ex.: "assume que é sobre o cliente X daqui pra frente"), não é conhecimento durável; "org" se o fato é sobre a PRICETAX em si (cargo, processo interno, conceito, produto — vale pra qualquer projeto); "project" se é específico deste cliente/projeto (a maioria dos casos, e quase sempre o certo pra um conflito encontrado dentro de uma reunião). null pros outros tipos.'),
  knowledgeType: z.enum(['FACT', 'DECISION', 'PREFERENCE', 'RULE', 'HYPOTHESIS', 'PROCEDURE', 'DEFINITION']).nullable().describe('SÓ pra type="save_knowledge_fact" ou "flag_knowledge_conflict": que TIPO de conhecimento é esse fato — FACT (um dado objetivo, ex.: "Felipe é o CEO"), DECISION (uma decisão tomada, ex.: "decidimos não levar a Unimed ao acordo coletivo"), PREFERENCE (uma preferência de alguém/do cliente, ex.: "esse cliente sempre prefere reunião às sextas"), RULE (uma regra/processo interno, ex.: "toda proposta de crédito acima de X precisa de validação da IVANA"), HYPOTHESIS (uma suposição ainda não confirmada, ex.: "acho que o atraso é por causa do fornecedor, mas não confirmei"), PROCEDURE (um passo a passo de como fazer algo), DEFINITION (o significado de um termo/sigla usado pelo cliente ou pela PRICETAX). null pros outros tipos.'),
  ...
  })).nullable().describe('SÓ pra type="save_knowledge_fact" ou "flag_knowledge_conflict": pessoas/empresas/projetos/temas que este fato menciona (ex.: o fato "Felipe é o CEO da PRICETAX" menciona a pessoa "Felipe" e a empresa "PRICETAX") — usado pra começar a conectar o conhecimento da RENATA num grafo simples de entidades. Vazio/null se não identificar nenhuma entidade clara, ou se for outro tipo — nunca force uma entidade genérica só pra preencher.'),
...
async function synthesizeAnswer({ question, chunks, history, projectSnapshot, factsText, context, personLookupText, calendarContextText, googleConnected }) {
  ...
  const response = await client.messages.parse({
    ...
    system: [{
      ...
      'Se duas fontes (dois trechos, ou um trecho contra o PERFIL DO PROJETO) trouxerem informação DIVERGENTE sobre o mesmo fato, NUNCA escolha uma versão silenciosamente — sinalize isso explicitamente numa seção type="warning" com título "Informação conflitante", liste as duas versões em items, e recomende validar com a pessoa certa antes de usar o dado (isso pode virar a recomendação também). Além da seção de aviso, PROPONHA também type="flag_knowledge_conflict" (ver descrição dos campos) pra esse conflito virar um registro rastreável na Central de Conhecimento (aba Conflitos), em vez de aparecer só nesta resposta e sumir depois — sempre que o conflito for um fato específico e verificável (não uma divergência vaga de opinião). NUNCA proponha isso ao mesmo tempo que outra ação nesta resposta (só uma proposedAction por vez) — se a pergunta do usuário já pedir claramente outra ação, priorize ela e deixe o conflito só no aviso desta vez.',
      ...
      'type="flag_knowledge_conflict" é PARECIDO com save_knowledge_fact, mas pra quando VOCÊ (não o usuário) percebe que duas fontes já existentes (dois trechos de reunião, ou um trecho contra o PERFIL DO PROJETO) se contradizem sobre o mesmo fato — normalmente ao lado de uma seção "warning" de informação conflitante. subject é o assunto em comum; content é a primeira versão; conflictingContent é a segunda, contraditória; cada uma DEVE citar a fonte dentro do próprio texto (reunião, data, quem falou) pra ficar rastreável depois. Ao confirmar, as DUAS versões viram fatos registrados, já marcados como conflitantes entre si — ninguém escolhe uma sozinha, é exatamente o mesmo princípio de nunca resolver uma divergência por conta própria, só que agora fica registrado em vez de se perder quando a conversa terminar.',
...
export async function askProjectAssistant({ pool, orgId, projectId, userId, question, context, projectData, projectUpdatedAt }) {
  ...
  if (errorMsg) {
    ...
    } else if (rawAction && rawAction.type === 'flag_knowledge_conflict') {
      // Mesma defesa dos 3 campos obrigatórios de save_knowledge_fact,
      // mais conflictingContent (a segunda versão) — sem os dois lados
      // não dá pra registrar o conflito de verdade.
      if ((rawAction.subject || '').trim() && (rawAction.content || '').trim() && (rawAction.conflictingContent || '').trim() && rawAction.scope) {
        proposedAction = { ...rawAction, sourceMeetingId: (context && context.meetingId) || null };
        logMetric(pool, { orgId, projectId, eventType: 'conflict_flagged_from_answer', metadata: { subject: rawAction.subject, scope: rawAction.scope } }).catch(() => {});
      } else {
        console.error('Assistente do Projeto: propôs flag_knowledge_conflict incompleto — descartada.', rawAction);
      }
    }
...
// efeito pra type="save_knowledge_fact"/"flag_knowledge_conflict": a IA
// sugere um escopo, mas quem decide de fato é o usuário no seletor do
// painel (src/assistant/ProjectAssistant.jsx) antes de clicar
// Confirmar. Nunca confia cegamente no valor recebido — revalida contra
// o enum real antes de aplicar (defesa em profundidade, mesmo padrão de
// todo id proposto pela IA neste arquivo).
const OVERRIDABLE_SCOPES = new Set(['conversation', 'project', 'org']);
const KNOWLEDGE_ACTION_TYPES = new Set(['save_knowledge_fact', 'flag_knowledge_conflict']);
...
export async function decideProposedAction(pool, orgId, projectId, userId, messageId, decision, actingUserName, overrides) {
  ...
  if (decision === 'reject') {
    ...
    } else if (baseAction && baseAction.type === 'flag_knowledge_conflict') {
      logMetric(pool, { orgId, projectId, eventType: 'conflict_flag_rejected', metadata: { subject: baseAction.subject } }).catch(() => {});
    }
    return { actionStatus: 'rejected' };
  }
  const finalAction = overrides && baseAction && KNOWLEDGE_ACTION_TYPES.has(baseAction.type) && OVERRIDABLE_SCOPES.has(overrides.scope)
